...
### En god verdi i et histogram vil være at det er lett å lese den. Det skal være like mellomrom og det skal være enkelt å se på histogrammet for å finne informasjonen.
```
...
```{r}
colnames(df)=c("naering", "sysselsetningsgruppe","statistikkvariabel","aar","verdi")
df %>%
filter(sysselsetningsgruppe >= 20)+
  filter(aar >= 2015) +
  filter(naering %in% c("Industri", "Tjenesteyting", "Andre naeringer")) +
  ggplot(aes(verdi/1000, fill = naering)) +
